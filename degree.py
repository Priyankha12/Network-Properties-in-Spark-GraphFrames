...
def simple(g):
	...
	edges_flipped = g.edges.select(g.edges['dst'], g.edges['src'])

	# Combine old and new edges. Distinctify to eliminate multi-edges
	total_edges = g.edges.unionAll(edges_flipped).distinct()
	# Filter to eliminate self-loops.
	# A multigraph with loops will be closured to a simple graph
	simple_edges = total_edges.filter('src != dst')
	# If we try to undirect an undirected graph, no harm done
	return GraphFrame(g.vertices, simple_edges)
def degreedist(g):
	# Generate a DF with degree,count
	return g.inDegrees.selectExpr('id as id', 'inDegree as degree').groupBy('degree').count()
...
def readFile(filename, large, sqlContext=sqlContext):
	...
	if large:
		...
		# Strip off header row.
            ...
            delim=","
	lines = lines.map(lambda x: x.split(delim))
        #encoded_lines = lines.map(lambda x: x.encode("ascii", "ignore").split())
		# Extract pairs from input file and convert to data frame matching
        # schema for graphframe edges.
        edges = sqlContext.createDataFrame(lines,["src","dst"])
	...
	vertices =  edges.selectExpr('src as id').unionAll(edges.selectExpr('dst as id')).distinct()
        #vertices = sqlContext.createDataFrame(encoded_lines.flatMap(lambda x: [[x[0]],[x[1]]]).collect(),["id"]).dropDuplicates()
...
if len(sys.argv) > 1:
	...
	print("Writing distribution to file " + out + ".csv")
   	...
   	f=pd.read_csv('amazon.graph.large.csv')
    	...
    	f=pd.read_csv('amazon.graph.small.csv')
    	print('Amazon small')
    	results = powerlaw.Fit((f['count']))
    	print(results.power_law.alpha)
